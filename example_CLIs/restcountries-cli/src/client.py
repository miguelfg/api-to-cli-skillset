...
class APIClient:
    """HTTP client for REST Countries API Python Client - Product Requirements Document."""

    def __init__(self, config: Config):
        self.config = config
        self.base_url = (
            config.get('base_url')
            or config.get('RESTCOUNTRIES_BASE_URL')
            or os.getenv('RESTCOUNTRIES_BASE_URL')
            or 'https://restcountries.com'
        )
