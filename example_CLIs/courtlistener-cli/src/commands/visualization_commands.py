"""Click commands for visualization resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_visualization(limit, offset, output_format, output_path):
    """List visualization"""
    ...
    try:
        result = client.get('/visualization/', params=params)
...
@click.argument('visualization_id', type=int)
def get_visualization(visualization_id):
    """Get visualization by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/visualization/({id})/')
