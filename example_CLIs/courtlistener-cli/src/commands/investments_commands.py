"""Click commands for investments resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_investments(limit, offset, output_format, output_path):
    """List investments"""
    ...
    try:
        result = client.get('/investments/', params=params)
...
@investments.command('get')
@click.argument('investments_id', type=int)
def get_investments(investments_id):
    """Get investments by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/investments/({id})/')
