"""Click commands for docket-entries resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_docket_entries(limit, offset, output_format, output_path):
    """List docket-entries"""
...
@click.argument('docket_entries_id', type=int)
def get_docket_entries(docket_entries_id):
    """Get docket-entries by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/docket-entries/({id})/')
