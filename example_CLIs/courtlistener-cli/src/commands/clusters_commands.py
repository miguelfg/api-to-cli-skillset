"""Click commands for clusters resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_clusters(limit, offset, output_format, output_path):
    """List clusters"""
...
@click.argument('clusters_id', type=int)
def get_clusters(clusters_id):
    """Get clusters by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/clusters/({id})/')
