"""Click commands for docket-alerts resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_docket_alerts(limit, offset, output_format, output_path):
    """List docket-alerts"""
    ...
    try:
        result = client.get('/docket-alerts/', params=params)
...
@docket_alerts.command('get')
@click.argument('docket_alerts_id', type=int)
def get_docket_alerts(docket_alerts_id):
    """Get docket-alerts by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/docket-alerts/({id})/')
