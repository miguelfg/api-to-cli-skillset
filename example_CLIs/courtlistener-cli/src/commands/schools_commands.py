"""Click commands for schools resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_schools(limit, offset, output_format, output_path):
    """List schools"""
    ...
    try:
        result = client.get('/schools/', params=params)
...
@schools.command('get')
@click.argument('schools_id', type=int)
def get_schools(schools_id):
    """Get schools by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/schools/({id})/')
