"""Click commands for attorneys resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_attorneys(limit, offset, output_format, output_path):
    """List attorneys"""
    ...
    try:
        result = client.get('/attorneys/', params=params)
...
@click.argument('attorneys_id', type=int)
def get_attorneys(attorneys_id):
    """Get attorneys by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/attorneys/({id})/')
