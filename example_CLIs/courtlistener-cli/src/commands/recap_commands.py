"""Click commands for recap resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_recap(limit, offset, output_format, output_path):
    """List recap"""
    ...
    try:
        result = client.get('/recap/', params=params)
...
@recap.command('get')
@click.argument('recap_id', type=int)
def get_recap(recap_id):
    """Get recap by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/recap/({id})/')
