"""Click commands for prayers resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_prayers(limit, offset, output_format, output_path):
    """List prayers"""
    ...
    try:
        result = client.get('/prayers/', params=params)
...
@prayers.command('get')
@click.argument('prayers_id', type=int)
def get_prayers(prayers_id):
    """Get prayers by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/prayers/({id})/')
