"""Click commands for debts resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_debts(limit, offset, output_format, output_path):
    """List debts"""
    ...
    try:
        result = client.get('/debts/', params=params)
...
@debts.command('get')
@click.argument('debts_id', type=int)
def get_debts(debts_id):
    """Get debts by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/debts/({id})/')
