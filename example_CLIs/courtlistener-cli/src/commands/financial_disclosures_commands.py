"""Click commands for financial-disclosures resource"""
...
def financial_disclosures():
    """FinancialDisclosures management"""
    pass
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_financial_disclosures(limit, offset, output_format, output_path):
...
@financial_disclosures.command('get')
@click.argument('financial_disclosures_id', type=int)
def get_financial_disclosures(financial_disclosures_id):
    """Get financial-disclosures by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/financial-disclosures/({id})/')
