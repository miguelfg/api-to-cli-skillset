"""Click commands for recap-documents resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_recap_documents(limit, offset, output_format, output_path):
    ...
    try:
        result = client.get('/recap-documents/', params=params)
...
@click.argument('recap_documents_id', type=int)
def get_recap_documents(recap_documents_id):
    """Get recap-documents by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/recap-documents/({id})/')
