"""Click commands for alerts resource"""
...
@click.option('--output', 'output_path', default='./output',
              type=click.Path())
def list_alerts(limit, offset, output_format, output_path):
    """List alerts"""
    ...
    try:
        result = client.get('/alerts/', params=params)
...
@alerts.command('get')
@click.argument('alerts_id', type=int)
def get_alerts(alerts_id):
    """Get alerts by ID"""
    client = CourtListenerClient()
    
    try:
        result = client.get('/alerts/({id})/')
