# open_meteo Python Client - Product Requirements Document
...
- Example:
```bash
python -m src.cli batch --input-file data/batch.csv --format json --output-path ./output
```

## 7. Input/Output Examples
### Single Command Example
```bash
python -m src.cli forecast list --format json
```
...
### Export Example
```bash
python -m src.cli forecast list --format xlsx --output-file output/forecast.xlsx
```
...
### Typical Workflow
1. `uv sync`
2. `python -m src.cli --help`
3. `python -m src.cli <resource> <command> ...`
