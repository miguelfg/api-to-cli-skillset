---
name: prd-to-cli
description: Generate a production-ready Python Click CLI project from a PRD.md file about an API. Takes a Product Requirements Document describing an API client and generates a complete project structure with one Click subcommand per API resource, batch request processing (CSV/TXT), configuration management (.env), and output formatting options (JSON/CSV/XLSX). Reads implementation decisions (including HTTP library) from the PRD produced by doc-to-prd. Use when converting API documentation into an executable CLI tool.
triggers:
...
**Input Decisions:**
This skill reads implementation decisions from the PRD (generated by `doc-to-prd`), including:
- Project/CLI name
- HTTP library choice (`requests`, `httpx`, `aiohttp`, `urllib3`)
- Logging preferences
- Batch input/output preferences
- Timestamp behavior
...
The skill will:
1. **Read decisions from PRD** (including HTTP library and runtime preferences).
...
### Step 2: Validate PRD Decisions

Before code generation, validate PRD includes:
- `HTTP Library` decision
- Base URL and authentication strategy
- Resource inventory and endpoint list
- Output and batch preferences
...
🔄 **Configurable options** — Read preferences from PRD decisions, not hardcoded
🐍 **HTTP library selection** — Implement client/tests from PRD choice (requests/httpx/aiohttp/urllib3)
...
HTTP library must be selected in `doc-to-prd` and stored in PRD:
...
The generated `src/client.py`, `requirements.txt`, and `pyproject.toml` are configured for the selected library.
